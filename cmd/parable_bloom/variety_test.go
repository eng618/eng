...
func avgVineLength(vines []Vine) float64 {
	total := 0
	for _, v := range vines {
		total += v.Length()
	}
...
func avgTurnCount(vines []Vine) float64 {
	total := 0
	for _, v := range vines {
		if v.Length() < 3 {
			continue
...
func avgDistanceToCenter(vines []Vine, gridSize [2]int) float64 {
	total := 0.0
	count := 0
...
func TestVarietyProfile_LengthMixInfluence(t *testing.T) {
	...
	longProf := VarietyProfile{LengthMix: map[string]float64{"short": 0.1, "medium": 0.3, "long": 0.6}, TurnMix: 0.2}
	shortProf := VarietyProfile{LengthMix: map[string]float64{"short": 0.6, "medium": 0.3, "long": 0.1}, TurnMix: 0.6}
...
func TestVarietyProfile_TurnMixInfluence(t *testing.T) {
	...
	spec := DifficultySpecs["Seedling"]
	cfg := GeneratorConfig{MaxSeedRetries: 50, LocalRepairRadius: 3, RepairRetries: 4}

	highTurn := VarietyProfile{LengthMix: map[string]float64{"short": 0.3, "medium": 0.4, "long": 0.3}, TurnMix: 0.9}
	lowTurn := VarietyProfile{LengthMix: map[string]float64{"short": 0.3, "medium": 0.4, "long": 0.3}, TurnMix: 0.1}
...
func TestVarietyProfile_RegionBias(t *testing.T) {
	gridSize := [2]int{12, 8}
	spec := DifficultySpecs["Sprout"]
	cfg := GeneratorConfig{MaxSeedRetries: 50, LocalRepairRadius: 3, RepairRetries: 4}

	edgeProf := VarietyProfile{RegionBias: "edge"}
	centerProf := VarietyProfile{RegionBias: "center"}
