...
func init() {
	...
	ParableBloomCmd.AddCommand(LevelRepairCmd)
}
...
func repairFileIfNeeded(path, idStr string, overwrite, dryRun, verbose bool) (bool, error) {
	...
	level := generateLevel(id, fmt.Sprintf("Level %d", id), DifficultyForLevel(id, nil), 0, 0, verbose, 0, false)
	...
	err = WriteLevel(path, level, overwrite)
	if err != nil {
