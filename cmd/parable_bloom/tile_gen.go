...
func GrowFromSeed(seed Point, occupied map[string]bool, gridSize [2]int, targetLen int, profile VarietyProfile, _ GeneratorConfig, rng *rand.Rand) (Vine, map[string]bool, error) {
	...
	for len(path) < targetLen {
		...
		if len(path) == 1 && len(profile.DirBalance) > 0 {
			...
			dx := head.X - prev.X
			dy := head.Y - prev.Y
			straight := Point{X: head.X + dx, Y: head.Y + dy}
...
func chooseNeighborByDirBias(origin Point, neighbors []Point, dirBalance map[string]float64, rng *rand.Rand) Point {
...
func pickSeedWithRegionBias(w, h int, occ map[string]bool, profile VarietyProfile, rng *rand.Rand) *Point {
