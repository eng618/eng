...
`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		var err error
		if file != "" {
			level, err = ReadLevel(file)
			if err != nil {
				log.Error("Failed to read level file: %v", err)
				return
			}
		} else if id != 0 {
			path := GetLevelFilePath(id, "assets/levels")
			if !FileExists(path) {
				log.Error("Level file not found: %s", path)
				return
			}
			level, err = ReadLevel(path)
			if err != nil {
				log.Error("Failed to read level file: %v", err)
				return
			}
...
func init() {
	...
	ParableBloomCmd.AddCommand(LevelRenderCmd)
}

// renderLevelToWriter prints a visual representation of a level to the given writer.
// style can be "ascii" or "unicode".
func renderLevelToWriter(w io.Writer, level *Level, style string, showCoords bool) {
...
func buildOccupancy(level *Level, width, height int) map[string][]struct{ vineIdx, segIdx int } {
...
func computeCellGlyph(level *Level, occ map[string][]struct{ vineIdx, segIdx int }, x, y int, style, emptyCell string, headMap map[string]string) string {
...
func headGlyph(vine Vine, j int, headMap map[string]string) (string, bool) {
	if j == 0 {
...
func straightGlyph(style string, curr, prev, next *Point) (string, bool) {
...
func tailGlyph(style string, prev, next *Point) (string, bool) {
