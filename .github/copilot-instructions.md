...
- Validate: `task validate` runs format + lint + test — **always run before commits**.
...
- Modules: Keep feature code in `cmd/<feature>/` and shared helpers in `internal/utils/` or feature-scoped packages under `internal/utils/`.
- Tests: Use `testify` and prefer table-driven tests. When asserting logs, swap writers via `log.SetWriters` and `log.ResetWriters`.
- Imports: Use `github.com/eng618/eng/internal/utils` for the utils package; alias sub-packages when needed (e.g., `configUtils "github.com/eng618/eng/internal/utils/config"`).

## Patterns & Examples

- Spawn a child process with proper signal/IO handling:
  - Use `utils.StartChildProcess(exec.Command("tool", "args"...))` from [internal/utils/command.go](internal/utils/command.go).
- Bitwarden integration:
  - Use `utils.EnsureBitwardenSession()` and helpers in [internal/utils/bitwarden.go](internal/utils/bitwarden.go) to read/store secrets; do not parse `bw` JSON manually.
- Config keys stability:
  - Add migrations in [internal/utils/config/migration.go](internal/utils/config/migration.go) when renaming keys; call `viper.WriteConfig()` after changes.
- Multi-repo git ops:
...
- Adding a new command:
  1. Create `cmd/<feature>/<feature>.go` with a `*Cmd` variable and `init()` registering subcommands.
  2. Register the parent command in [cmd/root.go](cmd/root.go) via `rootCmd.AddCommand(...)`.
  3. Add `--info` flag pattern for commands that manage config (see [cmd/git/git.go](cmd/git/git.go#L24-L39) for example).
...
- Wire logs via `internal/utils/log`; respect `--verbose`.
