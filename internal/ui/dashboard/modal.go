...
func (m Model) renderHelpModal() string {
	...
	fmt.Fprintf(&b, "  %s   %s\n", keyStyle.Render("F     "), descStyle.Render("Force fetch (git fetch --all --prune --force, overwrites tags)"))
	fmt.Fprintf(&b, "  %s   %s\n", keyStyle.Render("p     "), descStyle.Render("Pull (git pull --rebase --autostash)"))
	fmt.Fprintf(&b, "  %s   %s\n", keyStyle.Render("P     "), descStyle.Render("Force pull (fetch --force then pull, overwrites tags)"))
	fmt.Fprintf(&b, "  %s   %s\n", keyStyle.Render("s     "), descStyle.Render("Sync (fetch, pull --rebase)"))
	fmt.Fprintf(&b, "  %s   %s\n", keyStyle.Render("S     "), descStyle.Render("Force sync (fetch --force then pull, overwrites tags)"))
